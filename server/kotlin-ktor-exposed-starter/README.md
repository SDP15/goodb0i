...
`GET /stock` --> get all widgets in the database

`GET /stock/{id}` --> get one stock instance by id (integer)

`POST /stock` --> add a new stock to the database by providing a JSON object (converted to a NewWidget instance).
e.g - 
...
`PUT /stock` --> update an existing widgets name or quantity. Pass in the id in the JSON request to determine which record to update

`DELETE /stock/{id}` --> delete the stock with the specified id
